...
def main(cfg):
    ...
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=1, pin_memory=True, shuffle=False, num_workers=8, drop_last=False
    )
            ...
            if (total_step > 0) and (total_step % cfg.val_frequency == 0):
                ...
                with torch.no_grad():
                    for vdata in val_pbar:
                        ...
                        disp_gt_v_s = disp_gt_v.squeeze(1)
                        valid_mask = valid_mask & (disp_gt_v_s.abs() < 192)
                        ...
                        out_counts = (out * valid_mask).reshape(out.shape[0], -1).sum(dim=1)
