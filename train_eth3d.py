...
def _repair_batchnorm_stats(model):
    repaired = 0
    for m in model.modules():
        if not isinstance(m, torch.nn.modules.batchnorm._BatchNorm):
            continue
        if hasattr(m, 'running_mean') and m.running_mean is not None:
            if torch.isnan(m.running_mean).any() or torch.isinf(m.running_mean).any():
                m.running_mean = torch.where(
                    torch.isfinite(m.running_mean), m.running_mean, torch.zeros_like(m.running_mean)
                )
                repaired += 1
        if hasattr(m, 'running_var') and m.running_var is not None:
            if torch.isnan(m.running_var).any() or torch.isinf(m.running_var).any():
                m.running_var = torch.where(
                    torch.isfinite(m.running_var), m.running_var, torch.ones_like(m.running_var)
                )
                repaired += 1
    return repaired
...
def main(cfg):
    ...
    if isinstance(resume_ckpt, str) and resume_ckpt.endswith(".pth") and os.path.exists(resume_ckpt):
        ...
        repaired = _repair_batchnorm_stats(model)
        if repaired > 0:
            logger.warning(f"Repaired NaN/Inf BN stats in {repaired} layer(s) after checkpoint load.")
    ...
    while should_keep_training:
        ...
        for data in pbar:
            ...
            if (total_step > 0) and (total_step % cfg.val_frequency == 0):
                ...
                total_epe, total_out, elem_num = 0.0, 0.0, 0
                ...
                with torch.no_grad():
                    for vdata in val_pbar:
                        ...
                        out = (epe > float(cfg.val_outlier_threshold)).float()
                        ...
                        occ_mask = torch.from_numpy(np.array(occ_mask)).to(valid_v.device)
                        valid_mask = (valid_v >= 0.5) & (occ_mask == 255)

                        if valid_mask.sum() > 0:
                            local_epe = epe[valid_mask].mean()
                            local_out = out[valid_mask].mean()
                        else:
                            local_epe = torch.zeros((), device=epe.device)
                            local_out = torch.zeros((), device=epe.device)

                        if not torch.isfinite(local_epe):
                            local_epe = torch.zeros((), device=epe.device)
                            local_out = torch.zeros((), device=epe.device)
                        ...
                        if accelerator.is_main_process:
                            ...
                            if elem_num > 0:
                                val_pbar.set_postfix(epe=(total_epe / elem_num),
                                                     d1=100.0 * (total_out / elem_num))
                ...
                if accelerator.is_main_process and elem_num > 0:
                    accelerator.log({'val/epe': total_epe / elem_num,
                                     'val/d1': 100.0 * total_out / elem_num}, total_step)
