...
def gray_2_colormap_np(img, cmap='rainbow', max=None):
    ...
    cmap_m = matplotlib.colormaps.get_cmap(cmap)  # avoid the deprecated get_cmap API
...
def maybe_visualize(cfg, accelerator, step, tensors, save_dir):
    """
    tensors: dict with keys possibly in ['disp_pred', 'disp_gt', 'left', 'right']
             values are tensors whose first dim is batch
    """
    ...
    dest = str(viz_cfg.get('to', 'disk')).lower()  # 'wandb' | 'disk' | 'both'
...
def main(cfg):
    ...
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=8, pin_memory=True, shuffle=False, num_workers=8, drop_last=False
    )
    ...
    if isinstance(resume_ckpt, str) and resume_ckpt.endswith(".pth") and os.path.exists(resume_ckpt):
        ...
        logger.info(f"Loading fine-tuning weights from {cfg.restore_ckpt}")
    ...
    while should_keep_training:
        ...
        for data in pbar:
            ...
            tensors_for_viz = {
                'disp_pred': disp_preds[-1],  # B,1,H,W
                'disp_gt': disp_gt,  # B,1,H,W
                'left': left,  # B,3,H,W
                'right': right,  # B,3,H,W
            }
            ...
            if (total_step > 0) and (total_step % cfg.val_frequency == 0):
                ...
                total_image_epe, total_images = 0.0, 0.0
                total_out, total_pixels = 0.0, 0.0
                val_pbar = tqdm(val_loader, dynamic_ncols=True, disable=not accelerator.is_main_process)

                with torch.no_grad():
                    for vdata in val_pbar:
                        _, lval, rval, disp_gt_v, valid_v = [x for x in vdata]
                        ...
                        assert disp_pred_v.shape == disp_gt_v.shape, (disp_pred_v.shape, disp_gt_v.shape)
                        epe = torch.abs(disp_pred_v - disp_gt_v).squeeze(1)
                        disp_gt_v = torch.squeeze(disp_gt_v, dim=1)

                        valid_mask = valid_v >= 0.5
                        if valid_mask.dim() == 4:
                            valid_mask = valid_mask.squeeze(1)
                        valid_mask = valid_mask & (disp_gt_v.abs() < 192)

                        flat_epe = epe.reshape(epe.shape[0], -1)
                        flat_valid = valid_mask.reshape(valid_mask.shape[0], -1)
                        pixel_counts = flat_valid.sum(dim=1)
                        nonempty = pixel_counts > 0
                        image_epe = (flat_epe * flat_valid).sum(dim=1) / pixel_counts.clamp_min(1)
                        out_counts = ((epe > 3.0) & valid_mask).reshape(epe.shape[0], -1).sum(dim=1)

                        batch_stats = torch.stack([
                            image_epe,
                            nonempty.to(dtype=epe.dtype),
                            out_counts.to(dtype=epe.dtype),
                            pixel_counts.to(dtype=epe.dtype),
                        ], dim=1)
                        stats = accelerator.gather_for_metrics(batch_stats)
                        if stats.dim() == 1:
                            stats = stats.unsqueeze(0)

                        total_image_epe += float(stats[:, 0].sum())
                        total_images += float(stats[:, 1].sum())
                        total_out += float(stats[:, 2].sum())
                        total_pixels += float(stats[:, 3].sum())

                        if accelerator.is_main_process and total_images > 0 and total_pixels > 0:
                            val_pbar.set_postfix(epe=(total_image_epe / total_images),
                                                 d1=100.0 * (total_out / total_pixels))
                ...
                if total_images > 0 and total_pixels > 0:
                    accelerator.log({'val/epe': total_image_epe / total_images,
                                     'val/d1': 100 * total_out / total_pixels}, total_step)
